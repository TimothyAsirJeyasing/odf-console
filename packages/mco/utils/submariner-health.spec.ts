...
describe('shouldRunPrePairValidation', () => {
  it('runs only for a valid Data Foundation cluster pair', () => {
    expect(shouldRunPrePairValidation(MAX_ALLOWED_CLUSTERS, true, true)).toBe(
      true
    );
    expect(
      shouldRunPrePairValidation(MAX_ALLOWED_CLUSTERS - 1, true, true)
    ).toBe(false);
    expect(shouldRunPrePairValidation(MAX_ALLOWED_CLUSTERS, false, true)).toBe(
      false
    );
    expect(shouldRunPrePairValidation(MAX_ALLOWED_CLUSTERS, true, false)).toBe(
      false
    );
  });
});
